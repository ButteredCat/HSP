...
class IRawData {
 public:
  explicit IRawData(const std::string& raw_data, const DecoderPtr& aux_decoder)
      : raw_data_{raw_data}, aux_dec{aux_decoder} {};
  virtual void traverse() = 0;
  virtual void decode(const std::string& dst_file, int begin, int end) = 0;
  ...
  IRawData& set_frame_aux_size(int value) {
    if (value < 0) {
      // throw std::exception("must not be negative");
    }
    frame_aux_size_ = value;
    return *this;
  }
