...
void load(const std::string &file_path, cv::Mat output) {
    auto poDataset = GDALDatasetUniquePtr(
        GDALDataset::FromHandle(GDALOpen(file_path.c_str(), GA_ReadOnly)));
...
void read(const std::string &file_path) {
    // auto poDataset =
    // GDALDatasetUniquePtr(GDALDataset::FromHandle(GDALOpen(file_path.c_str(),
    // GA_ReadOnly)));
    auto poDriver = GetGDALDriverManager()->GetDriverByName("ENVI");
    auto dataset = GDALDatasetUniquePtr(GDALDataset::FromHandle(
        poDriver->Create(file_path.c_str(), 10, 10, 10, GDT_UInt16, nullptr)));
